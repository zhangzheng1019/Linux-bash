...
if [ $flag != "0" ]; then
		...
		`scp -r $backup_dir nms@$remoteIp:/home/backup/nms_mysqlbackup`
		flag=`echo $?`
		if [ $flag == "0" ];then
			echo "$backup_dir transport complete!"
		else
			echo "$backup_dir transport failed!"
		fi
        else
